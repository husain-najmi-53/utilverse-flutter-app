...
class CalculatorModule extends StatefulWidget {
  const CalculatorModule({Key? key}) : super(key: key);
...
class _CalculatorModuleState extends State<CalculatorModule> {
  ...
  void equalPressed() {
  if (userInput.isEmpty || !_isValidExpression()) {
...
bool _isValidExpression() {
  // Check if there are at least 3 characters in the input
  ...
  if (isOperator(userInput.substring(userInput.length - 1))) {
    return false;
  }
...
bool containsOperator() {
  // Check if the input contains any of the operators
